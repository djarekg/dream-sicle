...
export default defineConfig({
  plugins: [
    {
      ...
      configureServer(server) {
        server.middlewares.use(async (req, res, next) => {
          ...
          try {
            const apiServerModule = await server.ssrLoadModule("/src/server.ts");
            const nodeAdapterModule = await server.ssrLoadModule("/src/utils/node-http-adapter.ts");
            ...
            const message = error instanceof Error ? error.message : "Failed to handle API request";
  ...
  optimizeDeps: {
    exclude: ["@ds/contracts", "@ds/db", "@ds/utils"],
  },
  resolve: {
    alias: {
